...
router.post('/', async function addService(req: Request<{}, {}, IBareServiceItem>, res) {
    try {
...
router.get('/', async function getAllServices(req, res) {
    try {
        const servicesList = await db.query(`SELECT * FROM services`)
...
router.get('/:id(\\d+)', async function getServicesByClinicId(req, res) {
    ...
    try {
        const {
            rows
        } = await db.query(`SELECT s.title, s.description
                            FROM services AS s
...
router.put('/:id(\\d+)', async function updateServiceById(req: Request<{id: string}, {}, IServiceItem>, res) {
    try {
